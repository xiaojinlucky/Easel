...
from easel.skill_route import route as route_skills, suggest_recipe, summarize as skill_summarize, SKILL_ROOTS
from easel import skill_state as skillstate
...
from clipper_api import router as clipper_router
try:
...
# agent_options 不用这份导入：由文件顶部的 easel.model_runtime 提供（带 EASEL_THINKING_LEVEL 覆盖，
# 与 /api/model-settings 用的 easel.model_settings_api 读同一套档案）。
from easel.runtime import openclaw_command, runtime_env, require_subscription, CREATE_FLAGS, PROFILE, run_agent_command, abort_session, agent_reply, native_image_ready, SHARED_RAW_STREAM as LOCAL_RAW_STREAM
...
# 上游只列了 7860；本机的工作台与采集扩展跑在 7870（extensions/easel-clipper/manifest.json 的
# host_permissions 也写死 7870），两个端口都要放行，否则真前端的每次写操作都会吃 403。
...
def _loopback_peer(request: Request) -> bool:
    ...
    try:
        ...
        return False
# 默认只绑回环。要从别的设备访问工作台，显式设 EASEL_HOST=0.0.0.0（或局域网 IP）再启动，
# 而不是改代码：放开绑定时拦住「局域网内无人值守写操作」的，是下面的 local_write_guard。
# 服务管理器另用 uvicorn --host 127.0.0.1，路径保持不变。
...
# 放开局域网访问时要一并放行 Host，否则用 http://192.168.x.x:7870 打开会被 400 挡在门口。
...
# settings_router 用文件顶部导入的 easel.model_settings_api（换型号才再验证的实现），
# 这里不再从 web.settings_api 覆盖同名变量——上游那份没有 determined_models()。
...
def get_skills() -> list[dict]:
    env = _read_env()
    result = []
    blocked = skillstate.disabled_names()
    for root in SKILL_ROOTS:
        if not root.is_dir():
            continue
...
async def api_skill_set_enabled(name: str, req: SkillEnabledRequest):
    full = find_skill(name)
    if full is None:
        raise HTTPException(404, f"SKILL '{name}' 不存在")
    skillstate.set_enabled(full, bool(req.enabled))
    return {"ok": True, "name": full, "enabled": skillstate.is_enabled(full)}
...
async def api_skill_detail(name: str):
    ...
    if full is None:
        raise HTTPException(404, f"SKILL '{name}' 不存在")
    md = _skill_md_path(full)
    if md is None:
        raise HTTPException(404, f"SKILL '{name}' 不存在")
    desc, layer, body = _parse_skill_md(md)
...
def _chat_message(req: ChatRequest, turn_context: dict | None = None) -> str:
    ...
    return chat_turn_message(
        message, req.persona, active, stage=req.stage, route_query=req.message.strip(), pin=req.skill,
    )
...
async def api_skill(req: SkillRequest):
    ...
    if not skillstate.is_enabled(skill_full):
        raise HTTPException(403, f"技能「{skill_full}」在备选归档中，启用后才能运行")
...
async def _run_skill_node(node: dict, text: str, persona: str | None, session_id: str) -> dict:
    ...
    if not skillstate.is_enabled(skill_full):
        return {"id": node["id"], "title": title, "skill": skill_full, "ok": False, "output": "", "error": f"技能「{skill_full}」在备选归档中，启用后才能运行"}
...
async def api_output_delete(path: str):
    """删除内容库里的单个文件或整个项目目录。_sessions/_login 等系统顶层不可删。"""
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("EASEL_PORT", "7870"))  # 产品合同 7870：与 easel-clipper 扩展硬编码一致
    ...
    if proxy_url:
        print(f"  {proxy_url}")
    print()
    try:  # 扩展宿主只投递任务，模型通道在本进程：开机接着排干上次没跑完的加工队列
        from easel import research_enrich
        research_enrich.start_worker()
    except Exception as exc:  # noqa: BLE001  队列起不来不能挡工作台启动
        print(f"  · 素材库加工队列未启动：{exc}")
