// FX-09 数据卡 —— 玻璃面板里 4 条进度条生长 + 数字滚动（全部帧驱动）
// 来源：chart-animation 技能（值缓动 / tabular-nums / 计数器规则）
...
export const FxDataCard: React.FC = () => {
  const frame = useCurrentFrame();
  const zoom = interpolate(frame, [0, 150], [1, 1.035]);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#04060b' }}>
      <AbsoluteFill style={{ transform: `scale(${zoom})` }}>
        <OffthreadVideo src={staticFile('proxy.mp4')} startFrom={8 * 30} />
      </AbsoluteFill>
      <Dim v={0.3} />
      ...
      <Vignette />
      <Tag text="FX-09 · 数据卡 · chart-animation 技能" />
    </AbsoluteFill>
  );
};
