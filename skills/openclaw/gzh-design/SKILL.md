---
name: gzh-design
description: 微信公众号文章排版引擎：把 Markdown / Word(.docx) / PDF / 纯文本转成可直接粘贴进公众号编辑器的 HTML，自动章节编号、关键词标记、引言卡、目录、代码块、图片/GIF、作者签名；主题从 references/theme-index.md 组件库按题材推荐，支持"一键自动排版"与按描述/参考图生成自定义主题。触发：用户提到"公众号排版/微信排版/gzh"、想把文章(md/docx/pdf/纯文本)转成公众号 HTML、说"自动排版/一键排版"公众号内容。不用于普通网页/落地页/PPT。
layer: produce
---
