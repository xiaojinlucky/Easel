...
export const genericAdapter: Adapter = {
  ...
  async process(context) {
    ...
    const singlePage = process.env.EASEL_RESEARCH_SINGLE_PAGE === "1";
    if (!singlePage) {
    ...
    const converted = await convertHtmlToMarkdown(snapshot.html, snapshot.finalUrl, {
      enableRemoteMarkdownFallback: false,
    ...
    const document = {
      ...
      metadata: {
        ...
        samplingMode: singlePage ? "single_page_no_scroll" : "bounded_scroll",
      },
