...
export function cleanHtml(
  ...
  if (onlyMainContent) {
    ...
    const xhsProfile = /(^|\.)xiaohongshu\.com$/.test(pageUrl.hostname) && pageUrl.pathname.startsWith("/user/profile/");
    // XHS note cards use .footer for their title and visible like count.
    const protectedSelectors = xhsProfile ? [...FORCE_INCLUDE_SELECTORS, ".note-item .footer"] : FORCE_INCLUDE_SELECTORS;
    removeWithProtection(document, NAVIGATION_SELECTORS, protectedSelectors);
