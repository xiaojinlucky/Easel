...
本项目已安装官方 CLI，示例中的 `postiz` 统一使用 `python -m easel.postiz`。该入口使用本项目的本地 Postiz 账号和 `http://localhost:4007/api`，隔离其他应用的 OAuth 配置。不要再次全局安装、使用云端默认地址，或打印凭据。先运行 `python -m easel.postiz auth:status` 检查连接。媒体先上传到本地 Postiz；创建草稿也需要用户已连接的真实频道。发布、定时和对外消息必须有当前会话的明确授权，安装工作台不代表授权发布。
