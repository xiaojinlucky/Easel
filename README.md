...
启动 Web 工作台后访问 `http://localhost:7870`。安装完成后可以运行 `easel doctor` 检查环境，
...
| 命令 | 作用 |
|---|---|
| `easel web [--port 7870]` | 启动 Web 工作台 |
...
Easel 使用独立的 `easel` OpenClaw profile，不会覆盖你本机已有的 OpenClaw 配置。Web 默认运行在 `7870`，gateway 默认运行在 `18789`。
