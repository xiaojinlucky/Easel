...
def test_clipper_keeps_legacy_url_key_without_migration(local_state):
    ...
    with research.connection() as db:
        # 显式列名：现役树的 sources 已经长到 19 列，按位置塞值以后每加一列就会把这条测试弄坏
        db.execute('INSERT INTO sources (id,url,title,platform,topic,content,state,error,captured_at,asset_path,method) '
                   'VALUES (?,?,?,?,?,?,?,?,?,?,?)',
                   (identifier, url, '旧版选区', '浏览器采集', '', content, 'saved', '', 1.0, f'研究素材/{identifier}.md', research.CLIPPER_METHOD))
